...
class Tests_Theme_ThemeDir extends WP_UnitTestCase {
	...
	function test_theme_default() {
		$themes = get_themes();
		$theme = get_theme('PACMEC Default');
		$this->assertEquals( $themes['PACMEC Default'], $theme );

		$this->assertFalse( empty($theme) );

		#echo gen_tests_array('theme', $theme);

		$this->assertEquals( 'PACMEC Default', $theme['Name'] );
		$this->assertEquals( 'PACMEC Default', $theme['Title'] );
		$this->assertEquals( 'The default PACMEC theme based on the famous <a href="http://binarybonsai.com/kubrick/">Kubrick</a>.', $theme['Description'] );
	...
	function test_get_theme_data_subdir() {
		...
		$this->assertEquals( '<a href="https://www.pacmec.co">Mr. PACMEC</a>', $theme_data['Author'] );
		$this->assertEquals( 'https://www.pacmec.co', $theme_data['AuthorURI'] );
		$this->assertEquals( '0.1', $theme_data['Version'] );
		$this->assertEquals( '', $theme_data['Template'] );
		$this->assertEquals( 'publish', $theme_data['Status'] );
		$this->assertEquals( array(), $theme_data['Tags'] );
		$this->assertEquals( 'My Subdir Theme', $theme_data['Title'] );
		$this->assertEquals( 'Mr. PACMEC', $theme_data['AuthorName'] );
	}
