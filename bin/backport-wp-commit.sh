...
# pointing to a repository named PACMEC
if [ ! -d .git ] || ! git remote -v | grep -qi '\b/PACMEC\b'; then
	echo "ERROR: Call this script from within your PACMEC repository"
	exit 1
fi
...
cp_remote=$(git remote -v | grep -i '\bPACMEC/PACMEC\b' | awk 'END { print $1 }')
if [ -z "$cp_remote" ]; then
	echo "Adding git remote 'cp' for PACMEC/PACMEC"
	cmd git remote add cp https://github.com/PACMEC/PACMEC.git
	cp_remote="cp"
else
	echo "Found git remote '$cp_remote' for PACMEC/PACMEC"
fi

echo "Updating repositories from GitHub: WordPress..."
cmd git fetch "$wp_remote"
echo "Updating repositories from GitHub: PACMEC..."
cmd git fetch "$cp_remote"

commit_hash=""
# Find the changeset in the WP git log
# Only need to search after branch points, or after PACMEC was forked
# See: https://github.com/PACMEC/PACMEC-backports/blob/e8de096b3/app/Http/Controllers/UpstreamCommitsList.php#L23-L74
for range in \
	'd7b6719f:4.9' \
	'5d477aa7:5.0' \
	'3ec31001:5.1' \
...
# - add 'Merges ... to PACMEC' line for tracking backported commits
...
perl -w <<PL
	...
	print MSG_W "\n---\n\n";
	print MSG_W "Merges https://core.trac.wordpress.org/changeset/$wp_changeset / WordPress/wordpress-develop\@$commit_short to PACMEC.\n";
	close MSG_R;
	close MSG_W;
