...
class Tests_Basic extends WP_UnitTestCase {
	...
	function test_license_cp_copyright_years() {
		...
		// Check PACMEC copyright years
		preg_match(
			'#Copyright © 2018-(\d+) PACMEC and contributors#',
			$license,
			$matches
		);
		$this->assertNotEmpty( $matches );
...
<p>If a new user is created by PACMEC, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)</p>
