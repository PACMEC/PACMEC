...
class Tests_Term_WpInsertTerm extends WP_UnitTestCase {
	...
	public function test_wp_insert_term_unslash_name() {
		...
		$found = wp_insert_term( 'Let\\\'s all say \\"Hooray\\" for PACMEC taxonomy', 'wptests_tax' );

		$term = get_term( $found['term_id'], 'wptests_tax' );
		_unregister_taxonomy( 'wptests_tax' );

		$this->assertSame( 'Let\'s all say "Hooray" for PACMEC taxonomy', $term->name );
	}

	public function test_wp_insert_term_unslash_description() {
		register_taxonomy( 'wptests_tax', 'post' );
		$found = wp_insert_term( 'Quality', 'wptests_tax', array(
			'description' => 'Let\\\'s all say \\"Hooray\\" for PACMEC taxonomy',
		) );

		$term = get_term( $found['term_id'], 'wptests_tax' );
		_unregister_taxonomy( 'wptests_tax' );

		$this->assertSame( 'Let\'s all say "Hooray" for PACMEC taxonomy', $term->description );
	}
	...
	public function test_wp_insert_term_should_clean_term_cache() {
		...
		/**
		 * It doesn't appear that PACMEC itself ever sets these
